...
from mudpeter.pipeline import (
    fetch_publications_from_pubmed,
    enrich_publications_with_fulltext,
)
from mudpeter.db import (
    make_sqlite_url,
    get_engine,
    init_db as _init_db,
    upsert_publications,
...
def run(
    ...
    init_db: Annotated[
        bool, typer.Option("--init-db/--no-init-db", help="Create tables if needed")
    ] = True,  # this is probably not necessary for sqlite!
    dry_run: Annotated[
        bool,
        typer.Option("--dry-run", help="Fetch/enrich but do not write to database"),
    ] = False,
) -> None:
    """
...
def export(
    ...
    keyword: Annotated[
        str | None, typer.Option("--keyword", "-k", help="Filter by keyword")
    ] = None,
    afterdate: Annotated[
        str,
        typer.Option(
            "--afterdate",
            "-a",
            help="Only select full-texts updated after the given date or timepoint. Only formats `%Y-%m-%d` or `%Y-%m-%d %H:%M:%S` are accepted.",
        ),
    ] = None,
    beforedate: Annotated[
        str,
        typer.Option(
            "--beforedate",
            "-b",
            help="Only select full-texts updated before the given date or timepoint. Only formats `%Y-%m-%d` or `%Y-%m-%d %H:%M:%S` are accepted.",
        ),
    ] = None,
    oneshot: Annotated[
        bool,
        typer.Option(
            "--oneshot/--no-oneshot",
            "-o/-O",
            help="Create csv file for oneshot batch-text query.",
        ),
    ] = False,
    csv_sep: Annotated[
        str, typer.Option("--csv_sep", "-s", help="Separator for oneshot-csv-file.")
    ] = ";",
    instructions: Annotated[
        ...
        typer.Option(
            ...
            dir_okay=False,
            readable=True,
            help="Path to instructions file (for oneshot call).",
        ),
    ] = None,
    question: Annotated[
        Path | None,
        typer.Option(
            ...
            dir_okay=False,
            readable=True,
            help="Path to question file (for oneshot call).",
        ),
    ] = None,
    kw_specific: Annotated[
        bool,
        typer.Option(
            "--kw_specific/--not_kw_specific",
            "-w/-W",
            help="Create keyword specific instruction and question files for oneshot.",
        ),
    ] = False,
):
    """
    ...
    if afterdate:
        ...
        if not ad:
            console.print(
                f"[yellow]String {afterdate} could not be converted to date or datetime. Ignored.[/yellow]"
            )
    ...
    if beforedate:
        ...
        if not bd:
            console.print(
                f"[yellow]String {beforedate} could not be converted to date or datetime. Ignored.[/yellow]"
            )
    ...
    try:
        fulltext_files = export_fulltexts(
            export_directory, engine, keyword=keyword, beforedate=bd, afterdate=ad
        )
        console.print(
            f"[green]Full texts were successfully exported into directory {export_directory}.[/green]"
        )
        if oneshot:
            create_oneshot_csv(
                export_directory,
                contexts=fulltext_files,
                question=question,
                instructions=instructions,
                sep=csv_sep,
                keyword_specific=kw_specific,
            )
            console.print(
                f"[green]Oneshot csv file was placed into {export_directory}.[/green]"
            )
    except Exception as e:
